...
global.itemGet = ITEM
if (ITEM != item[? "currency"])global.descriptionTimer = room_speed * 4
if (ITEM = item[? "currency"])
{
	mod_variable_set("mod", "main", "coins", 1 + mod_variable_get("mod", "main", "coins"));
...
if "tag" in self
{
	...
	switch tag
	{
		...
		case "test"   : tem = item[? "brooch"] // this is for testing
								    break;
		case "coin"   : tem = item[? "currency"] // this is for testing
								    break;
		case "none"   :
